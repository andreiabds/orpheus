...
def create_df_music_features_lyrics_labels():
    ...
    df1_correct = pd.read_csv('data/spotify_correct_albums_IDs.csv')
    df2_features = pd.read_csv('data/spotify_audio_features.csv')
    df3_lyrics = pd.read_csv('data/stlyrics_musical_tracks_url.csv').dropna()
...
def build_nn_dataset():
    df = pd.read_csv('data/COMPLETE_DATA.csv', index_col=0)
